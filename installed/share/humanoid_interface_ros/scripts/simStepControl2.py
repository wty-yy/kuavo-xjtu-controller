...
def publish_single_step():
    ...
    def publish_poses(pose):
        torso_pos = np.asarray(pose[:3])
        torso_yaw = np.radians(pose[3])
        l_foot, r_foot = generate_steps(torso_pos, torso_yaw, 0.1)
        
        l_foot = [*l_foot[:3], torso_yaw]
        r_foot = [*r_foot[:3], torso_yaw]
        
        publish_foot_pose(0, l_foot, [*torso_pos, torso_yaw])
        publish_foot_pose(1, r_foot, [*torso_pos, torso_yaw])
